...
from llama_tools_jaliya import server
...
class TestRunLlamaServer(unittest.TestCase):
    ...
    @patch("llama_tools_jaliya.server.os.path.isfile", return_value=False)
    ...
    @patch("llama_tools_jaliya.server.subprocess.run", side_effect=CalledProcessError(1, "cmd"))
    @patch("llama_tools_jaliya.server.os.path.isfile", return_value=True)
    def test_run_llama_server_called_process_error(self, mock_isfile, mock_run):
        with self.assertRaises(CalledProcessError):
            run_llama_server("model.gguf")

    @patch("llama_tools_jaliya.server.run_llama_server")
